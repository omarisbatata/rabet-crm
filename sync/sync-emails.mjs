...
// Supabase via the crm_insert_inbound_email() RPC using the service_role key
// (server-side secret — this script never runs in a browser and the key is
// never shipped to app.js). See schema-emails.sql for the security model.
//
...
//
// Dedupe happens in crm_insert_inbound_email() via
// `on conflict (gmail_message_id) do nothing`, so re-processing the buffer
// window on every run is safe and just a no-op for messages we already have.
...
const SYNC_START_DATE = required('SYNC_START_DATE') // e.g. '2026-07-08' — no import before this date
...
async function insertInboundEmail(parsed, fallbackId) {
  ...
  const result = await supabaseRest('rpc/crm_insert_inbound_email', {
    method: 'POST',
    body: JSON.stringify({ p_data: payload }),
  })

  return result !== null // RPC returns the new row id, or null if deduped
}
